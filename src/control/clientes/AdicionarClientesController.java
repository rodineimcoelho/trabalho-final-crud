...
public class AdicionarClientesController implements Initializable {
    ...
    void adicionarCliente(ActionEvent event) {
        ...
        if(!checkEmpty(nome, txt_nome, label)){
            if(!checkEmpty(rua, txt_rua, label)){
                if(!checkEmpty(uf, box_uf, label)){
                    if(!checkEmpty(cidade, txt_cidade, label)){
                        if(!(checkEmpty(cepString, txt_cep, label))){
                            if(!cepString.matches("^[0-9]*$")){
                                ...
                                if(!checkEmpty(nomeUsuario, txt_nome_usuario, label)){
                                    if(!checkEmpty(senha, txt_senha, label)){
    ...
    private boolean checkEmpty(String string, TextField textField, Label label){
        if(string.isBlank()){
            label.setText("Rua não pode estar vazia");
            label.setTextFill(Paint.valueOf("red"));
            textField.setStyle(
                    "-fx-background-color: red, #FFFFFF ;\n" +
                            "    -fx-background-insets: 0, 0 0 1 0 ;\n" +
    ...
    private boolean checkEmpty(String string, ComboBox<String> comboBox, Label label){
        if(string.isBlank()){
            label.setText("Rua não pode estar vazia");
            label.setTextFill(Paint.valueOf("red"));
            comboBox.setStyle(
